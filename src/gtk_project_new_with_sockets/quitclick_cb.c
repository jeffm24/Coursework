...
void quitclick_cb(GtkWidget *w, gpointer data)
{
  ...
  g_signal_connect(G_OBJECT(quitWin), "delete_event", G_CALLBACK(closepopup_cb), NULL);
  g_signal_connect(G_OBJECT(quitWin), "destroy", G_CALLBACK(closepopup_cb), NULL);
  ...
  g_signal_connect(G_OBJECT(yes_btn), "clicked", G_CALLBACK(closepopup_cb), NULL);
